...
install the following package: express, multer, session, nodemon, body-parser, bcryptjs 
...
Instruction for Shippers: There are 2 types of shippers: Administrator: People who assign orders to Delivery Men and Delivery Men: People who do the delivery. To assign orders as an administrator, first click on the home icon on the account page (or localhost:3500/shipping_management ), then click on the order id container (not the link) in the table, the order id will automatically be filled in the form, then select delivery man. As a delivery man, you can can change the status of your assigned orders when you have successfully delivered it. By clicking on the order number link, you will then be redirected to Order detail page where there will be a select list for you to update your order status.
